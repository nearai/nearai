...
deploy() {
  ...
  docker build -f aws_runner/Dockerfile --platform linux/amd64 --build-arg FRAMEWORK=${FRAMEWORK} --build-arg TS_RUNNER=${TS_RUNNER} --provenance=false -t nearai-runner${FRAMEWORK} .
