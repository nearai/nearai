...
class CreateThreadAndRunRequest(BaseModel):
    ...
    thread: Optional[str] = Field(
        None, description='An OpenAI compatibility alias for environment. If no thread is provided, an empty thread will be created.'
    )
...
def create_environment_and_run(body: CreateThreadAndRunRequest, auth: AuthToken = Depends(get_current_user)) -> str:
    ...
    result = wrapper.invoke_function("agent-runner-docker",
                        {"agents": agents, "environment_id": environment_id,
                         "auth": auth.json(), "new_message": new_message})
    return result
