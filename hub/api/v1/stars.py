...
async def add_star(auth: NearAuthToken = Depends(get_auth), namespace: str = Form(...), name: str = Form(...)):
...
async def remove_star(auth: NearAuthToken = Depends(get_auth), namespace: str = Form(...), name: str = Form(...)):
    with get_session() as session:
        result = session.exec(
            select(Stars).where(Stars.account_id == auth.account_id, Stars.namespace == namespace, Stars.name == name)
        ).first()
