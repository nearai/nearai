...
def delegate(delegate_account_id: str, expires_at: datetime, auth: NearAuthToken = Depends(get_auth)):
...
def list_delegation(auth: NearAuthToken = Depends(get_auth)) -> List[Delegation]:
    with get_session() as session:
