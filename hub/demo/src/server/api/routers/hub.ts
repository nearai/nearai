...
export const hubRouter = createTRPCRouter({
  ...
  agentChat: protectedProcedure
    ...
    .mutation(async ({ctx, input}) => {
      ...
      if(!responseText.match(/".*\/.*\/.*/)) {  // check whether the response matches namespace/name/version
      ...
      const u2 =
        `${env.ROUTER_URL}/registry/download_file`;
      ...
      return await fetch(u2, {
        ...
        .then((tarReader) => {
          ...
          const parsedChat = rawChat.split("\n")
            .filter(message => message)
            .map(message => {console.log(message); return JSON.parse(message)});
          ...
          for (const fileInfo of tarReader.fileInfos) {
            ...
            if(fileInfo.type === 48) { // files are coming through as 48
              const originalFileName = fileInfo.name;
              const fileName = originalFileName.replace(/^\.\//, "");
              if(fileName !== "chat.txt" && fileName !== ".next_action") {
                environment.fileStructure[fileName] = fileInfo;
                environment.files[fileName] = tarReader.getTextFile(originalFileName);
              }
            }
          }
          return environment;
