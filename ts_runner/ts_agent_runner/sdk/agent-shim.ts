...
class Environment {
  async write_file(
    ...
    write_to_disk: boolean = true
  ): Promise<FileObject> {
    return await globalEnv.client.write_file(
      filename,
      content,
      encoding,
      filetype
  ...
  async read_file(filename: string): Promise<string> {
    return new Promise(async (resolve, reject) => {
      if (
        globalEnv.client &&
        "list_files_from_thread" in globalEnv.client &&
        "read_file_by_id" in globalEnv.client
      ) {
  ...
  async completion(
    ...
    if (globalEnv.client && "completions" in globalEnv.client) {
      let raw_response = await globalEnv.client.completions(
        messages,
        model,
        max_tokens,
        temperature,
  ...
  async query_vector_store(
    ...
    return globalEnv.client.query_vector_store(
      vector_store_id,
      query,
      full_files
    );
  ...
  async create_vector_store(
    ...
    return globalEnv.client.create_vector_store(
      name,
      file_ids,
      expires_after,
      chunking_strategy,
  ...
  async upload_file(
    ...
    return globalEnv.client.upload_file(
      file_content,
      purpose,
      encoding,
      file_name,
  ...
  async generate_image(
    ...
    return await globalEnv.client.generate_image(prompt, options);
  }
