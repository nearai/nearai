...
type ImagesResponse = OpenAI.ImagesResponse;
...
export class NearAIClient {
  ...
  query_vector_store = async (
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Error querying vector store: ${response.status} ${response.statusText}`
        );
      }
  ...
  add_file_to_vector_store = async (
    ...
    return this.hub_client.beta.vectorStores.files.create(vector_store_id, {
      file_id,
    });
  };
  ...
  completions = async (
    ...
    return await this._run_inference_completions(
      messages,
      model,
      stream,
      temperature,
  ...
  add_reply = async (
    ...
    return this.hub_client.beta.threads.messages.create(this.thread_id, body);
  };
  ...
  generate_image = async (
    ...
    return await this.hub_client.images.generate({
      prompt: prompt,
      ...options,
    });
  };
  ...
  private _listMessages = async (
    ...
    let messages = await this.hub_client.beta.threads.messages.list(
